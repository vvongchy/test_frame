...
class RequestUnit:
    def send_request(self, method, url, datas, data_type, **kwargs):
        """
        :param method:
        :param url:
        :param datas:
        ...
        res = None
        if method == "get":
            res = req_session.request(method=method, url=url, params=datas, **kwargs)
        elif method == "post":
            if data_type == "json":
                res = req_session.request(method=method, url=url, json=datas, **kwargs)
            else:
                res = req_session.request(method=method, url=url, params=datas, **kwargs)
        elif method == "put":
            pass
...
if __name__ == '__main__':
    res = RequestUnit().send_request("post", "http://127.0.0.1:5000/test_post", {"num1": 1, "num2": 2}, data_type="not json")
    # req_session = requests.session()
